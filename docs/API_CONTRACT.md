...
**Admin has no equivalent payments endpoint — confirmed gap, not just a guess.** `GET /admin/jobs?status=completed` returns identical data for a paid and an unpaid completed job (verified against the two seeded demo jobs — no payment field on the job object at all), and the authoritative reference confirms no `GET /admin/payments` route exists. **The admin app cannot currently distinguish paid from unpaid completed jobs at all.** The Completed Jobs screen shows a single list with Mark Paid on every job rather than tabs that can't be populated correctly. Needs either a payment-status field added to the admin job list, or a dedicated admin payments-list endpoint.
